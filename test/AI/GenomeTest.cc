...
BOOST_FIXTURE_TEST_CASE(SimpleGenomeTest, F) {
    ...
    BOOST_ASSERT(16 == genome1.getNeurons().size());
    BOOST_ASSERT(15 == genome1.getConnections().size());


    Genome genome2(1, 1, 1);
    BOOST_ASSERT(3 == genome2.getNeurons().size());
    BOOST_ASSERT(2 == genome2.getConnections().size());
}

BOOST_FIXTURE_TEST_CASE(CopyingConstuctorTest, F) {
    Genome genome1(1, 1, 1);
    Genome genome2(genome1);
    BOOST_ASSERT(genome1 != genome2);
}

BOOST_FIXTURE_TEST_CASE(ComplexGenomeTest, F) {
    Genome genome(50, 10, 20);
    BOOST_ASSERT(50 + 10 + 20 == genome.getNeurons().size());
    BOOST_ASSERT(50 + 9 + 20 == genome.getConnections().size());
}
